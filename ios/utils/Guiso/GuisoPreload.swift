...
public class GuisoPreload: Runnable,Equatable {
   ...
   private var isFromBeign = false
    ...
    private func saveToMemoryCache(_ img:UIImage){
        ...
        if !sm { GuisoSaver.saveToMemoryCache(key: self.mKey, image: img)}
    }
    
    private func saveToMemoryCache(_ gif:AnimatedImage){
        if !mKey.isValidSignature() { return }
        let sm = mOptions.getSkipMemoryCache()
        if !sm { GuisoSaver.saveToMemoryCache(key: self.mKey, gif:gif) }
    }
    ...
    func loadFromMemoryImg() -> UIImage?{
        ...
        if !skipCache ,let img =  cache.get(self.mKey)  {
                return img
        }
        return nil
    }
    ...
    func loadFromMemoryAnim() -> AnimatedImage?{
        ...
        let skipCache = mOptions.getSkipMemoryCache()
        if !skipCache {
            if let animImg =  cache.get(self.mKey) {
                 return animImg
            }
        }
        return nil
