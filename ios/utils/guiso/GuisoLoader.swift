...
class GuisoLoader {
     ...
     func updateTargetResizeWebGif(){
         ...
         let request = getRequestWeb(mHeader?.getFields())
     ...
     func updateTargetResizeWebVideo(){
         guard let url = URL(string: mUrl),
             let fullSize =  ImageHelper.getVideoThumbnail(videoUrl: url,header: mHeader, second: mSecond,exact: mExactFrame),
     ...
     func updateTargetResizeWebAudio(){
         guard let url = URL(string: mUrl),
             let fullSize =  ImageHelper.getAudioArtWork(url,header: mHeader),
             let resized = self.resizeImage(fullSize)
             else { onLoadFailed()
                 return
             }
     ...
     func updateTargetResizeFileVideo(){
         guard let url = URL(string: mUrl),
             let fullSize = ImageHelper.getVideoThumbnail(videoUrl: url, header: nil,second: mSecond,exact: mExactFrame),
             let resized = self.resizeImage(fullSize)
          else { self.onLoadFailed()
                     return
              }
     ...
     func updateTargetResizeFileAudio(){
         guard let url = URL(string: mUrl),
              let fullSize = ImageHelper.getAudioArtWork(url,header: nil),
             let resized = self.resizeImage(fullSize)
           else { self.onLoadFailed()
                      return
               }
