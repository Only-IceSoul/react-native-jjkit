...
class GuisoRequestBuilder {
    ...
    private var mPriority: Guiso.Priority = .normal
    private var mHeader: GuisoHeader?
    public init(_ url:String) {
      mUrl = url
      mSignature = url
    }
    public init(_ url:String,header: GuisoHeader) {
        mUrl = url
        mSignature = url
        mHeader = header
    }
    public init(_ url:URL) {
        mUrl = url.absoluteString
        mSignature = mUrl
   }
   public init(_ url:URL,header: GuisoHeader) {
        mUrl = url.absoluteString
       mSignature = mUrl
       mHeader = header
   }
    ...
    public func into(_ target: ViewTarget) -> ViewTarget {
        Guiso.get().putLoad(self,target,mPriority)
        return target
    }
    ...
    func getHeader() -> GuisoHeader? {
        return mHeader
    }
    
    func getPriority() -> Guiso.Priority {
        return mPriority
    }
